  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
      <div className="mb-12 text-center">
        <h1 className="text-4xl font-bold tracking-tight text-gray-900 sm:text-5xl">AI API 额度</h1>
        <p className="mx-auto mt-4 max-w-lg text-lg text-gray-600">兼容 OpenAI 格式，按量付费，永不过期</p>
      </div>

      <h2 className="text-xl font-bold text-gray-800 mb-6">一次性购买</h2>
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-16">
        {normal.map((plan) => (
          <div key={plan.id} className={"card flex flex-col p-6 transition-shadow hover:shadow-lg " + (plan.id==="standard"?"ring-2 ring-blue-500":"")}>
            {plan.id==="standard" && <div className="-mt-8 mb-4 text-center"><span className="rounded-full bg-blue-600 px-4 py-1 text-xs font-semibold text-white">推荐</span></div>}
            <h3 className="text-lg font-bold text-gray-900">{plan.name}</h3>
            <p className="mt-1 text-sm text-gray-500">{plan.description}</p>
            <div className="mt-4 flex items-baseline gap-1">
              <span className="text-3xl font-bold text-gray-900">¥{plan.price}</span>
              <span className="text-sm text-gray-500">/ 一次</span>
            </div>
            <p className="mt-1 text-sm font-medium text-blue-600">{(plan.tokens/10000).toFixed(0)} 万 Tokens</p>
      ...
      {vip && (
        <>
        <h2 className="text-xl font-bold text-gray-800 mb-6">月度订阅</h2>
        <div className="flex justify-center">
          <div className="card p-8 max-w-sm ring-2 ring-purple-500">
            <div className="-mt-12 mb-4 text-center"><span className="rounded-full bg-purple-600 px-5 py-1.5 text-sm font-semibold text-white">VIP 月度</span></div>
            ...
            <div className="mt-6 text-center flex items-baseline justify-center gap-1">
              <span className="text-4xl font-bold text-gray-900">¥{vip.price}</span>
              <span className="text-sm text-gray-500">/ 月</span>
            </div>
            <p className="mt-1 text-center text-sm font-medium text-purple-600">{(vip.tokens/10000).toFixed(0)} 万 Tokens</p>

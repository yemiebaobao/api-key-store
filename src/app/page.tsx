  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
      <div className="mb-12 text-center">
        ...
        <p className="mx-auto mt-4 max-w-lg text-lg text-gray-600">OpenAI compatible API. Pay as you go, no expiry.</p>
      </div>

      <h2 className="text-xl font-bold text-gray-800 mb-6">One-time Purchase</h2>
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-16">
        {normal.map((plan) => (
          <div key={plan.id} className={"card flex flex-col p-6 transition-shadow hover:shadow-lg " + (plan.id==="standard"?"ring-2 ring-blue-500":"")}>
            {plan.id==="standard" && <div className="-mt-8 mb-4 text-center"><span className="rounded-full bg-blue-600 px-4 py-1 text-xs font-semibold text-white">BEST SELLER</span></div>}
      ...
      {vip && (
        <>
        <h2 className="text-xl font-bold text-gray-800 mb-6">Monthly Subscription</h2>
        <div className="flex justify-center">
          <div className="card p-8 max-w-sm ring-2 ring-purple-500">
            <div className="-mt-12 mb-4 text-center"><span className="rounded-full bg-purple-600 px-5 py-1.5 text-sm font-semibold text-white">VIP MONTHLY</span></div>
            ...
            <div className="mt-6 text-center flex items-baseline justify-center gap-1">
              <span className="text-4xl font-bold text-gray-900">{formatPrice(vip.price)}</span>
              <span className="text-sm text-gray-500">/ month</span>
            </div>
            <p className="mt-1 text-center text-sm font-medium text-purple-600">{(vip.tokens/10000).toFixed(0)}M Tokens / month</p>
